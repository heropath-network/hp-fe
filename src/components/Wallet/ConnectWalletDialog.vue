...
const wallets = computed<WalletItem[]>(() => [
  {
    name: "MetaMask",
    connector:
      connectors.value.find((c) => c.name === "MetaMask") ||
      connectors.value.find((c) => c.name === "Injected"),
    icon: MetaMaskIcon,
        ...
        <div
          ...
          @click="
            () => {
              ...
              connect({ connector: wallet.connector, chainId: chainId });
            }
          "
        >
          <div class="flex items-center">
            ...
            <span
              class="text-[16px] leading-6 font-semibold text-[var(--hp-white-color)]"
              >{{ wallet.name }}</span
            >
          </div>
          <span>
            ...
            <template v-else-if="isConnectedStatus(wallet.name)">
              <div
                class="inline-block h-2 w-2 rounded-full bg-[var(--hp-text-green)]"
              ></div>
            </template>
          </span>
        </div>
