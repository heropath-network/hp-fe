<template>
  <div class="flex h-full flex-col bg-[var(--hp-bg-dark)]">
    <div class="overflow-x-auto">
      <template v-if="marketType === 'perps'">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-800">
            ...
            <PositionRow v-for="position in positions" :key="position.id" :position="position" />
          </tbody>
        </table>
      </template>
      <template v-else>
        <table class="w-full text-sm">
          <thead class="border-b border-gray-800">
            <tr class="text-xs text-gray-400">
              ...
              <th class="px-4 py-3 text-left font-medium">Entry Price</th>
              <th class="px-4 py-3 text-left font-medium">Index Price</th>
              <th class="px-4 py-3 text-left font-medium">PNL</th>
              <th class="px-4 py-3 text-center font-medium">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-800">
