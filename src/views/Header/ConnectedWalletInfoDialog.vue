...
async function viewOnExplorer() {
  ...
  const url = chainBlockExplorer ? `${chainBlockExplorer}/address/${address.value}` : undefined
  if (url) {
    ...
    <template #default>
      <div class="mt-4 bg-[var(--hp-bg-light)] w-full h-[144px] p-4">
        ...
        <div class="flex items-center mt-4">
          ...
          <div class="ml-2 flex-1 min-w-0 font-[500] text-[var(--hp-white-color)] text-[16px] leading-[24px] break-all">
            {{ address }}
          </div>
        </div>
        <div class="mt-2 flex items-center">
          <div class="flex item-center cursor-pointer group" @click="copy(address!)">
            <span class="text-[14px] leading-5 group-hover:text-[var(--hp-primary-green)]">Copy Address</span>
            <BaseIcon
              name="copy"
              size="16"
          ...
          <div class="flex item-center cursor-pointer group ml-6" @click="viewOnExplorer">
            <span class="text-[14px] leading-5 group-hover:text-[var(--hp-primary-green)]">View on Explorer</span>
            <BaseIcon
              name="blockLink"
              size="16"
              class="ml-1 h-4 w-4 text-[var(--hp-text-color)] transition-colors group-hover:text-[var(--hp-primary-green)]"
