...
export function isMarketAvailableInFourMeme(market: string): boolean {
  const fourMemeMarket = getFourMemeMarket(market)
  return fourMemeMarket !== undefined
}

export function autoSelectOracle(market: string): ProjectId | null {
  const upperMarket = market.toUpperCase()

  if (isMarketAvailableInFourMeme(market)) {
    return ProjectId.FOUR_MEME
  }
...
export function getAvailableOraclesForMarket(market: string): ProjectId[] {
  ...
  if (isMarketAvailableInFourMeme(market)) {
    oracles.push(ProjectId.FOUR_MEME)
  }
